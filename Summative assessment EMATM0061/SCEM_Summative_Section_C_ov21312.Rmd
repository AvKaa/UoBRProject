...
The following report demonstrates a Random Forest predictive modelling on the dataset 'winequality-red'. The objective of the model is as follows:

- Observe the capabilities of the Random Forest model on both an imbalanced and balanced data set.
- Observe how the accuracy differs when the model is fed with different subsets of data as test sets with use of the k-fold cross validation technique.
- Evaluate the affects of hyperparameters tuning on the Random Forest model accuracy.
...
<<<<<<< HEAD
In the following section the hyperparameter tuning will be carried out using 2 different packages, the ranger package and the caret package.   
=======
As the 'randomForest()' package is computation expensive, the following section aims to explore hyperparameter tuning utilizing other Random Forest packages. 
...
A benchmark comparison of the RandomForest model using the microbenchmark() function is shown as following:
...
times = 25, unit = 's')
>>>>>>> 1f18e2c59f4da930ef8b3b978b5dc10a0966a530
...
```
The mean processing time for randomForest() is about 0.8 seconds. This is considered expensive for a dataframe with only 1600 rows. The ranger() function was designed for parallel processing. Hence, to demonstrate that ranger() is not just faster because of parallel processing but also a more efficient way of processing, the num.threads is set to 1. Results shows that the Ranger() package reduces the processing time by more than a half.
...
From computation of the Random Forest model on the wine quality dataset performed above it can be concluded that:
...
- Variable importance evaluation shows that the higher ranked variables also demonstrates a stronger relationship with the quality through the use of scatter plots (ie, strong trend line observed in alcohol vs quality, weak trend line observed in total sulfur dioxide vs quality).
- Hyperparameter tuning generally increases the accuracy of the model by 5% and demonstrates the importance of tailoring hyperparameters for the specific dataset.
- It can be observed that the Random Forest model works better on Binary classification than on multi-class prediction. Hence, it might be more suitable to evaluate other models when dealing with a biased dataset.
- The Ranger() version of Random Forest displays massive reduction in computational expense with a slight trade-off of accuracy. Thus, for larger datasets the Ranger() function should be considered over the original randomForest() function instead.
