...
```{r}
finance_data_original <- read.csv("C:/Users/aveka/OneDrive/Documents/UoB/SCEM/UoBRProject/finance_data_EMATM0061.csv")
...
finance_data %>% select(state_year_code,education_expenditure,health_expenditure,totals_savings) %>%
  head(3)
```
...
finance_data %>% select(state,year,education_expenditure,health_expenditure,totals_savings) %>%
  head(3)
```
...
```{r}
finance_data %>%
  filter(state==c("Louisiana","Montana","Mississippi","Kentucky")) %>% #filters out the four states
  ggplot()+
  geom_smooth(aes(x=year,y=totals_revenue/1e+06,color=state), na.rm=T, se=F) + #create geom smooth plot
  theme_bw()+
...
alaska_summary <- finance_data %>%
  group_by(decade) %>%
  filter(decade==c(1990,2000,2010)) %>%
